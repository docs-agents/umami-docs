...
Umami API Client 使用 TypeScript 构建，包含调用 Umami 中每个可用 API 端点的函数。

## 要求

- [Node.js](https://nodejs.org/) 18.18 或更高版本
...
访问 Umami Cloud 的更多详情可在 [API 密钥](/docs/cloud/api-key) 下找到。
...
执行 API 调用的用户的 `USER_ID`。权限限制将根据应用程序设置应用。
...
您 Umami API 的端点。示例：`https://{yourserver}/api/`
